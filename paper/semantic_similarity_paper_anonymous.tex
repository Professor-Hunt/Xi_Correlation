...
\begin{abstract}
We propose applying Chatterjee's rank correlation coefficient $\xi$ directly to embedding dimensions as a semantic similarity metric for dense vector representations. Applied to 384-dimensional BERT sentence embeddings, this \emph{dimensionwise} $\xi$ achieves Spearman correlation $\rho=0.859$ with human judgments on 1{,}500 STS-B pairs---within 0.86\% of cosine similarity ($\rho=0.867$). Mechanistic analysis reveals dimensionwise $\xi$ works via \emph{distributed signal aggregation}: approximately 25\% of dimensions contribute meaningfully, with no single dimension dominating. The method provides a conservative rank-based alternative to magnitude-based measures. We validate $\xi$'s theoretical foundations through synthetic experiments ($N=17{,}500$) showing near-perfect detection of nonlinear transformations ($\xi \geq 0.93$) where cosine fails ($\leq 0.12$), and demonstrate a projection-based formulation for stochastic embeddings. The empirical success suggests semantic similarity in BERT embeddings is encoded as rank correlation across neural feature activations. Complete open-source implementation is provided.
\end{abstract}
